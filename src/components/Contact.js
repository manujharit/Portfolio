        ...
        <div className="border flex flex-row rounded-3xl drop-shadow-md shadow-md shadow-gray-400 bg-blue-50 h-[350px] px-[7%] py-[10%]">
            <div className=" flex flex-col w-[100%] md:w-[60%]">
                ...
                <div className="flex flex-row">
                    {data.map(social => <a href={social.link} target='_blank' key={social.id} className="flex flex-row items-start justify-end text-center mx-[1%]" ><img src={CDN_URL + social.imageId} className="w-[20px] h-[20px] rounded-md mx-[5px]" /><span className={social.class}>{social.user}</span></a>)}
                </div>
            </div>
