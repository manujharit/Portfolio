  ...
  return (
    <div className="my-[12%] flex flex-col justify-center items-center">
      ...
      <div className="flex flex-wrap">
        {TechStack.map(tech => <div className="h-[100px] w-[100px] mx-[10%] my-[5%]"><img key={tech.id} src={CDN_URL + tech.main + '.png'} className="h-[100px] w-[100px] " alt={<ImageShimmer/>} onMouseEnter={() => handleMouseEnter(tech.id)} onMouseLeave={() => handleMouseLeave()} />{showCard === tech.id && <TechCard data={tech} />}</div>)}
      </div>
    </div>
  )
}
